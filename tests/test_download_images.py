...
class UnitTest(TestCase):
    def test__parse_blob_path(self):
        ...
        image_class, image_name = _parse_blob_path(
            "more_folders/another_folder/cat_1/image.png"
        )
        self.assertEqual(image_class, "cat_1")
        self.assertEqual(image_name, "image.png")

        image_class, image_name = _parse_blob_path(
            os.path.join("folder", "another_folder", "cat_1", "image.png")
        )
        self.assertEqual(image_class, "cat_1")
        self.assertEqual(image_name, "image.png")
    ...
    def test__create_paths_dataframe(self):
        ...
        blob_names = [
            os.path.join("cat_1", "blob_1"),
            os.path.join("cat_3", "blob_b"),
            os.path.join("folder", "cat_1", "blob_1"),
        ]
        blobs = []
        ...
        self.assertDictEqual(
            df.to_dict(),
            {
                ...
                "class": {
                    i: item for i, item in enumerate(["cat_1", "cat_3", "cat_1"])
                },
            },
        )
